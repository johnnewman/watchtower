...
# 2) Optional: To use a microcontroller, enable serial access via 'sudo raspiconfig'\n\
# 3) Optional: To use the HTTP API, upload SSL certificates to \"$WATCHTOWER_PATH/nginx/certs\"\n\
#              You will need:
                  - (1) a public SSL cert and (2) the corresponding private key. Used for encrypting traffic
                  - (3) a certificate authority cert for validating clients
